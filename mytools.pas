...
function Translit2Lat(const Str: string): string;
const
  RArrayL = 'абвгдеёжзийклмнопрстуфхцчшщьыъэюяіїє';
  RArrayU = 'АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЬЫЪЭЮЯІЇЄ';
  colChar = 36;
...
begin
  ...
  // Перетворюємо Unicode → Win1251 (ANSI)
  AnsiS := AnsiString(s);

  SetLength(ResultString, Length(AnsiS));

  for i := 1 to Length(AnsiS) do
  begin
    WinCode := Byte(AnsiS[i]);

    case WinCode of
      $C0..$DF: DosCode := WinCode - $30; // А..Я
      $E0..$EF: DosCode := WinCode - $10; // а..п
      178: DosCode := 179; // І
      179: DosCode := 178; // і
      170: DosCode := 244; // Є
      186: DosCode := 243; // є
    ...
    ResultString[i] := AnsiChar(DosCode);
  end;
...
begin
  // Unicode → CP1251
  Bytes1251 := TEncoding.GetEncoding(1251).GetBytes(S);

  // CP1251 → CP866
  SetLength(Bytes866, Length(Bytes1251));
  OemToCharBuffA(@Bytes1251[0], @Bytes866[0], Length(Bytes1251));

  SetString(Result, PAnsiChar(@Bytes866[0]), Length(Bytes866));
end;
...
var
  ...
  str:string;
begin
  // Перетворюємо Unicode → CP1251 (ANSI)
         str := trim(S);
         str := StringReplace(str, 'і', 'i', [rfReplaceAll]);
         str := StringReplace(str, 'І', 'I', [rfReplaceAll]);

  WinBytes := TEncoding.GetEncoding(1251).GetBytes(str);

  // Готуємо буфер DOS-866
  SetLength(DosBuf, Length(WinBytes));

  // CP1251 → DOS866
  if Length(WinBytes) > 0 then
    CharToOemBuffA(@WinBytes[0], @DosBuf[1], Length(WinBytes));
...
const
  ...
  ed:array [1..9] of string=('один','два','три','чотири','п`ять','шiсть','сiм','вiсiм','дев`ять');
  ed1:array [1..9] of string=('одна','двi','три','чотири','п`ять','шiсть','сiм','вiсiм','дев`ять');
  cat:array [1..10] of string=('десять','одиннадцять','дванадцять','тринадцять',
  'чотирнадцять','п`ятнадцять','шiстнадцять','сiмнадцять','вiсiмнадцять',
  'дев`ятнадцять');
  dec:array [1..8] of string=('двадцять','тридцять','сорок','п`ятдесят','шiстдесят',
  'сiмдесят','вiсiмдесят','дев`яносто');
  sto:array [1..9] of string=('сто','двiстi','триста','чотириста','п`ятсот','шiстсот','сiмсот',
  'вiсiмсот','дев`ятсот');
  tis:array [1..4] of string=('тисяч','мiльйонiв','мiльярдiв','трильонiв');
  tis1:array [1..4] of string=('тисяча','мiльйон','мiльярд','трильон');
  tis2:array [1..4] of string=('тисячi','мiльйони','мiльярди','трильони');
...
////////  char c;
  ...
  begin
    if (source[length(source)]='1') and (source[length(source)-1]<>'1') then
        result:=result+'ціла '+dr
   else result:=result+'цілих '+dr;

    if (dr_num[length(dr_num)]='1') and (dr_num[length(dr_num)-1]<>'1') then
       case length(dr_num) of
        1: result:=result+'десята';
        2: result:=result+'сота';
        3: result:=result+'тисячна';
       end
    else
       case length(dr_num) of
        1: result:=result+'десятих';
        2: result:=result+'сотих';
        3: result:=result+'тисячних';
       end
 end;
pp:=result[1];
...
{  sd:=TSaveDialog.Create(application);
  ...
  begin
    MessageDlg('Не могу создать файл ', mtError, [mbCancel], 0);
    exit;
  end;
