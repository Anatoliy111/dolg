...
begin
  ...
  // якщо ви використовуЇте сучасний Delphi (2009+), потр≥бно €вно привести до AnsiString
  // s := AnsiString(s);
   AnsiS := AnsiString(TEncoding.GetEncoding(1251).GetBytes(s));

  for i := 1 to Length(s)-1 do
  begin
    // Ѕеремо пор€дковий номер (код) символу в кодов≥й стор≥нц≥ Windows-1251
    WinCode := Ord(AnsiS[i]);

    // ѕерев≥р€Їмо д≥апазон, €кий потр≥бно перекодувати ( ирилиц€ у Win-1251)
    if (WinCode >= 192) and (WinCode <= 255) then
    begin
      // —пец≥альн≥ випадки (часто необх≥дн≥ дл€ украњнськоњ/б≥лоруськоњ)
      case WinCode of
        // ≤ велика (U+0406) -> код ѓ
        178: DosCode := 179; // '≤' (Win 178) -> '≤' (DOS 179)
        // ≥ мала (U+0456)
        179: DosCode := 178; // '≥' (Win 179) -> '≥' (DOS 178)
        // ™ велика (U+0404)
        170: DosCode := 244; // '™' (Win 170) -> '™' (DOS 244)
        // Ї мала (U+0454)
        186: DosCode := 243; // 'Ї' (Win 186) -> 'Ї' (DOS 243)
        // ѓ велика (U+0407)
        175: DosCode := 241; // 'ѓ' (Win 175) -> 'ѓ' (DOS 241)
        // њ мала (U+0457)
        191: DosCode := 242; // 'њ' (Win 191) -> 'њ' (DOS 242)
        // • велика (U+0490)
        165: DosCode := 245; // '•' (Win 165) -> '•' (DOS 245)
        // і мала (U+0491)
        180: DosCode := 246; // 'і' (Win 180) -> 'і' (DOS 246)

        // ќсновний д≥апазон кирилиц≥ (ј-я, а-п)
        192..223: DosCode := WinCode - 64; // ј (192) -> ј (128)
        // ќсновний д≥апазон кирилиц≥ (р-€)
        224..239: DosCode := WinCode - 16; // р (224) -> р (208)

        // —пец≥альн≥ символи дл€ DOS, €к≥ не потрапили у case (наприклад, і, ѓ, •)
        else
          DosCode := WinCode; // «алишаЇмо без зм≥н, €кщо немаЇ точного в≥дпов≥дника
      end;
    end
    else
    begin
      // ASCII символи (0-127) не зм≥нюютьс€
      DosCode := WinCode;
    end;

    // ѕеретворюЇмо код DOS назад у символ ≥ додаЇмо до результату
    c := Chr(DosCode);
